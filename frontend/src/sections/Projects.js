...
export const Projects = ({ projects = [] }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 h-full px-6 md:px-12 min-w-0">
      {/* List / Selector */}
      <div className="lg:w-[280px] lg:min-w-[280px] lg:max-w-[320px] flex flex-col gap-4 overflow-y-auto project-list-scrollbar pr-2 pt-8 shrink-0" style={{ height: 'calc(100vh - 140px)', maxHeight: 'calc(100vh - 140px)' }}>
      ...
      {/* Details View */}
      <div className="flex-1 min-w-0 relative overflow-y-auto project-details-scrollbar pt-8 pb-8" style={{ height: 'calc(100vh - 140px)', maxHeight: 'calc(100vh - 140px)' }}>
        <AnimatePresence mode="wait">
          {activeProject && (
            ...
            >
              ...
              <div className="p-6 md:p-8" style={{ width: '100%', minWidth: '450px' }}>
                <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4" style={{ width: '100%' }}>
                  <div style={{ minWidth: '300px', flex: '1 1 auto', maxWidth: 'calc(100% - 80px)' }}>
                    <h2 className="text-2xl md:text-3xl font-bold text-white mb-2 break-words" style={{ wordBreak: 'break-word', overflowWrap: 'break-word', width: '100%' }}>{activeProject.title}</h2>
                    <div className="flex items-center gap-4 text-xs font-mono text-textMuted flex-wrap">
                      <span className="flex items-center gap-1 whitespace-nowrap"><Building2 className="w-3 h-3" /> {activeProject.organisation}</span>
                      <span className="flex items-center gap-1 whitespace-nowrap"><Calendar className="w-3 h-3" /> {activeProject.start_date}</span>
                    </div>
                  </div>

                  <div className="flex gap-3 shrink-0" style={{ flexShrink: 0 }}>
                ...
                <div className="prose prose-invert prose-sm max-w-none text-textMuted leading-relaxed mb-8" style={{ width: '100%', minWidth: '400px' }}>
                  <p style={{ wordBreak: 'break-word', overflowWrap: 'break-word' }}>{activeProject.description}</p>
                </div>
